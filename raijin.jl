...
function positions(account_id::Int64, instrument::AbstractString, side::AbstractString, unit::Float64)
  ...
  try
    ...
    @time pdata, cv = simulation(instrument)
    if pdata == 0
      return 0
    end
  ...
  elseif side != "buy" && (center > pdata[end,P750]) && (center < average)
    ...
    println("$(now()): sell | $(side != "buy") | $(center > pdata[end,P750]) | $(center < average)")

    println("$(now()) : シミュレーション結果：通貨ペア（$instrument）現在値=$center | $average")
    println("$(now()) : 97.5% = $(pdata[end,P975])")
    println("$(now()) : 75.0% = $(pdata[end,P750])")
    println("$(now()) : 50.0% = $(pdata[end,P500])")
    println("$(now()) : 25.0% = $(pdata[end,P250])")
    println("$(now()) :  2.5% = $(pdata[end,P025])")
...
while true
  try
    ...
    if code == 0 || length(tr) > 0
      ...
      for i in unique(inst)
        code, pss = get_positions(account_id, i)
        if code == 14
